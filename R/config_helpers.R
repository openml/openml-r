...
checkConfig = function(conf) {
  ...
  ns2 = c("server", "apikey", "cachedir", "verbosity", "arff.reader", "confirm.upload", "download.method")
  if (any(ns %nin% ns2))
  ...
  assertChoice(conf$download.method, c("auto", "internal", "wininet", "libcurl", "wget", "curl"))
...
printableConfig = function(conf) {
  ...
  fmt = paste(
    ...
    "  apikey           : %s\n",
    sep = "\n")
  sprintf(fmt, conf$server, conf$cachedir, conf$verbosity, conf$arff.reader, conf$confirm.upload, conf$download.method, key)
}
