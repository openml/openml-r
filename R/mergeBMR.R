...
mergeBMR = function(...) {
  ...
  learner.names = unique(unlist(lapply(set, mlr::getBMRLearnerIds)))
  task.names = unique(unlist(lapply(set, mlr::getBMRTaskIds)))
  ...
  existing.combos = rbindlist(lapply(set, function(bmr) {
    mlr::getBMRAggrPerformances(bmr, as.df = TRUE)[, c("task.id", "learner.id")]
  }))
  ...
  # td = peelList(lapply(set, mlr::getBMRTaskDescriptions))
  ...
  measures.merged = peelList(lapply(set, mlr::getBMRMeasures))
  measures.merged = measures.merged[!duplicated(measures.merged)]
  
  # get all actual learners and merge
  lrns.merged = peelList(lapply(set, mlr::getBMRLearners))
