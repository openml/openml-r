...
populateOMLCache = function(dids = integer(0L), task.ids = integer(0L),
  ...
  downloadStuff = function(type, fun, ids) {
    if (length(ids) > 0L) {
      ...
      lapply(ids, fun, verbosity = verbosity)
    }
  }

  downloadStuff("datsets", getOMLDataSet, dids)
  downloadStuff("tasks", getOMLTask, task.ids)
  downloadStuff("flows", getOMLFlow, flow.ids)
  downloadStuff("runs", getOMLRun, run.ids)
