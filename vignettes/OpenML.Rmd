...
At the moment there is no [CRAN](https://cran.r-project.org/) release yet. To install the current development version use the [`devtools`](https://cran.r-project.org/web/packages/devtools/index.html) package and run
```{r eval=FALSE, message=FALSE}
devtools::install_github("openml/openml-r")
```
in an R session. Using the OpenML package also requires a reader for the [ARFF](http://www.cs.waikato.ac.nz/ml/weka/arff.html) file format. By default [RWeka](https://cran.r-project.org/web/packages/RWeka/index.html) is used. However, the [farff](https://github.com/mlr-org/farff) package can be used as an alternative. You can install the packages with the following calls.
```{r eval=FALSE, message=FALSE}
...
If you are not using `mlr`, you will have to invest some more time and effort to get things done. So, unless you have good reasons to do otherwise, we strongly encourage to use `mlr`.

In the case you want to work without `mlr`, you have to create some objects on your own. You will find detailed descriptions of the constructors in the manual.
...
A [flow](#flows) is an implementation of a single algorithm or a script. To create a flow, we can use the `mlr` package, which already contains numerous implemented algorithms. Each [`mlr` learner](http://mlr-org.github.io/mlr-tutorial/release/html/integrated_learners/) can be considered as an implementation of a flow, which then again can be uploaded to the server with the function `uploadOMLFlow`.
...
As mentioned before, it is possible to avoid using `mlr`, but it requires much more effort from your side.

### Upload an OpenML run to the server

In addition to uploading data sets or flows, one can also upload runs (which a priori have to created, e.g., using `mlr`):
```{r eval = FALSE}
...
* are missing a certain functionality
