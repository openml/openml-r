...
library("OpenML")
setOMLConfig(apikey = "c1994bdb7ecb3c6f3c8f3b35f4b47f1f", verbosity = 0)
```
...
```
Using the OpenML package also requires a reader for the [ARFF](http://www.cs.waikato.ac.nz/ml/weka/arff.html) file format. By default [farff](https://cran.r-project.org/web/packages/farff/index.html) is used. Alternatively, the [RWeka](https://cran.r-project.org/web/packages/RWeka/index.html) package can be used. You can install the packages with the following calls.
```{r eval=FALSE, message=FALSE}
...
You can also have a closer look at the data set on the corresponding OpenML web page (http://openml.org/d/`r subset(datasets, name == "iris" & number.of.classes == 3)$data.id`).
