downloadOpenMLTaskResults <- function(id, dir = getwd(), show.info = TRUE) {
  ...
  downloadAPICallFile(api.fun = "openml.task.results", file = fn.task.results, task_id = id, show.info = show.info)
...
parseOpenMLTaskResults <- function(file) {
  ...
  getMetrics <- function() {
    ns.ids <- getNodeSet(doc, "/oml:task_results/oml:results/oml:run/oml:run_id")
    run.ids <- unlist(lapply(ns.ids, function(x) xmlValue(x)))
    
    ns.impl.names <- getNodeSet(doc, "/oml:task_results/oml:results/oml:run/oml:implementation")
    impl.names <- unlist(lapply(ns.impl.names, function(x) xmlValue(x)))
    
    ns.metrics <- getNodeSet(doc, "/oml:task_results/oml:results/oml:run/oml:measure")
