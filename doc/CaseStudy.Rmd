...
Now we search for appropriate tasks on OpenML by querying the server using `listOMLTasks()`.

For this study, we look for tasks that meet the following criteria:  
1. Supervised classification problem  
2. 10-fold cross-validation as resampling procedure
3. Predictive accuracy as evaluation measure  
4. Binary classification problem
5. No missing values -- As Random Forest cannot handle them automatically  
6. Less than 500 instances -- keep evaluation time low  
5. $n < p$  
