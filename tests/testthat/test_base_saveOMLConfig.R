...
test_that("saveOMLConfig", {
  ...
  # write config in different file
  conf.file = tempfile()
  saveOMLConfig(arff.reader = "RWeka", verbosity = 2, path = conf.file)
  conf = as.list(loadOMLConfig(path = conf.file))
  expect_true(conf$arff.reader == "RWeka" && conf$verbosity == 2)
